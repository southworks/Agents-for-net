...
Include every blocking and required finding ID, invent no IDs, and attribute each action bullet to at least one supplied `MTAPI-####` ID. Use prose rather than action bullets when there is no applicable finding.
