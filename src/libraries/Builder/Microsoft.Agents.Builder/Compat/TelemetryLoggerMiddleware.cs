...
namespace Microsoft.Agents.Builder.Compat
{
    ...
    public class TelemetryLoggerMiddleware : IMiddleware
    {
        ...
        /// or threads to receive notice of cancellation.</param>
        /// <returns>A task that represents the work queued to execute.</returns>
        protected virtual async Task OnDeleteActivityAsync(IActivity activity, CancellationToken cancellation)
        {
        ...
        protected Task<Dictionary<string, string>> FillSendEventPropertiesAsync(IActivity activity, Dictionary<string, string> additionalProperties = null)
        {
            ...
            if (LogPersonalInformation)
            {
                ...
                {
                    properties.Add(TelemetryConstants.AttachmentsProperty, ProtocolJsonSerializer.ToJson(activity.Attachments));
                }
            }
...
#pragma warning disable CA1822 // Mark members as static (can't change this without breaking binary compat)
        protected Task<Dictionary<string, string>> FillDeleteEventPropertiesAsync(IActivity activity, Dictionary<string, string> additionalProperties = null)
        ...
        private static void PopulateAdditionalChannelProperties(IActivity activity, Dictionary<string, string> properties)
        {
            ...
            switch (activity.ChannelId)
            {
                case Channels.Msteams:
                    ...
                    {
                        properties.Add("TeamsTeamInfo", ProtocolJsonSerializer.ToJson(teamsChannelData.Team));
                    }
