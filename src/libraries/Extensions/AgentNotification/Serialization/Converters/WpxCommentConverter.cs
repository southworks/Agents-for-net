...
{
    internal class WpxCommentConverter : ConnectorConverter<WpxComment>
    {
        protected override void ReadExtensionData(ref Utf8JsonReader reader, WpxComment value, string propertyName, JsonSerializerOptions options)
        {
            value.Properties.Add(propertyName, JsonSerializer.Deserialize<JsonElement>(ref reader, options));
        }

        protected override bool TryReadExtensionData(ref Utf8JsonReader reader, WpxComment value, string propertyName, JsonSerializerOptions options)
        {
        ...
        protected override bool TryWriteExtensionData(Utf8JsonWriter writer, WpxComment value, string propertyName)
        {
            if (propertyName.Equals(nameof(value.Properties)))
            {
