...
namespace Microsoft.Agents.Extensions.Teams.AI.Clients
{
    ...
    public class LLMClient<TContent>
    {
        ...
        public void AddFunctionResultToHistory(ITurnState memory, string name, object results)
        {
            List<ChatMessage> history = memory.GetValue<List<ChatMessage>?>(this.Options.HistoryVariable) ?? new() { };
        ...
        )
        {
            ...
            IStreamingResponse? streamer = null;
            ...
            BeforeCompletionHandler handleBeforeCompletion = new((object sender, BeforeCompletionEventArgs args) =>
            {
                ...
                if (args.Streaming)
                {
                    ...
                    streamer = memory.Temp.GetValue<IStreamingResponse?>("streamer");
                    if (streamer == null)
                    {
                        // Create streamer and send initial message
                        streamer = context.StreamingResponse;
                        ...
                        {
                            streamer.QueueInformativeUpdateAsync(this._startStreamingMessage!);
                        }
                    }
                }
            });
            ...
            ChunkReceivedHandler handleChunkReceived = new((object sender, ChunkReceivedEventArgs args) =>
            {
                ...
                if (citations != null)
                {
                    streamer.AddCitations(citations);
                }
            ...
            try
            {
                ...
                if (inputMsgs == null)
                {
                    object? content = memory.GetValue<string>("input");
        ...
        private void AddMessagesToHistory(ITurnState memory, string variable, IEnumerable<ChatMessage> messages)
        {
            ...
            List<ChatMessage> history = memory.GetValue(variable, () => new List<ChatMessage>());
        ...
        private void AddMessageToHistory(ITurnState memory, string variable, ChatMessage message)
        {
