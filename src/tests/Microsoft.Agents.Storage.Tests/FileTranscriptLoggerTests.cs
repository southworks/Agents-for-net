...
{
    public class FileTranscriptLoggerTests : TranscriptBaseTests
    {
        public FileTranscriptLoggerTests()
            : base()
        {
            this.Store = new FileTranscriptLogger(Folder);
            var folder = Path.Combine(Path.GetTempPath(), nameof(FileTranscriptLoggerTests));
        ...
        {
            get { return Path.Combine(Path.GetTempPath(), nameof(FileTranscriptLoggerTests)); }
        }
