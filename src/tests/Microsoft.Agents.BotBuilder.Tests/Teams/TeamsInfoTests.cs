...
namespace Microsoft.Agents.BotBuilder.Tests.Teams
{
    public class TeamsInfoTests
    {
        ...
        private readonly RestTeamsConnectorClient ConnectorClient;


        public TeamsInfoTests()
        {
            //CustomHttpClient = new HttpClient(new RosterHttpMessageHandler());

            // Set a special base address so then we can make sure the connector client is honoring this http client.
            //CustomHttpClient.BaseAddress = new Uri(BaseUri);

            var httpFactory = new Mock<IHttpClientFactory>();
            httpFactory.Setup(a => a.CreateClient(It.IsAny<string>()))
                .Returns(new HttpClient(new RosterHttpMessageHandler()));

            ConnectorClient = new RestTeamsConnectorClient(new Uri("http://localhost/"), httpFactory.Object, null);
        }
        ...
        private class RosterHttpMessageHandler : HttpMessageHandler
        {
            protected async override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
            {
                ...
                else if (request.RequestUri.PathAndQuery.EndsWith("team-id/pagedmembers?pageSize=-1"))
                {
                ...
                else if (request.RequestUri.PathAndQuery.EndsWith("conversation-id/pagedmembers?pageSize=-1"))
                {
                    var content = new TeamsPagedMembersResult
                    {
                        Members =
